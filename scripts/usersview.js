...
class favorites {
  ...
  async add(userlogin) {
    try {
      const user = await search.getUser(userlogin)

      if (user.login == undefined) {
        throw new Error('Esse usuário não existe, tente novamente')
      }
  ...
  onRemove() {
    ...
    rows.forEach(row => {
      ...
      row.querySelector('button').addEventListener("click", () => {
          const deleteUser = confirm('Deseja mesmo apagar este usuário?')
